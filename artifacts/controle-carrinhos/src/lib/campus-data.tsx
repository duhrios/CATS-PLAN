...
export const initialReservations: Reservation[] = [
  ...
  { id: 4, teacher: "Caio Martins", segment: "Fundamental 1", subject: "Português", className: "6º ano A · Português", room: "Sala 08", period: "Tarde", date: todayISO(), start: "13:30", end: "15:10", cart: "Carrinho C", status: "Concluída", kind: "Aula", quantity: 25 },
  { id: 5, teacher: "Lívia Costa", segment: "Fundamental 2", subject: "História", className: "8º ano A · História", room: "Sala 18", period: "Manhã", date: addDaysISO(1), start: "08:00", end: "09:40", cart: "Carrinho C", status: "Confirmada", kind: "Aula", quantity: 30 },
];

export const initialCarts: Cart[] = [
  { id: "a", name: "Carrinho A", code: "A", prefix: "A", total: 60, available: 55, location: "Armário A · Bloco 1", status: "Pronto", lastCheck: "Hoje, 06:52", accent: "bg-[hsl(var(--primary))]", unavailable: false, unavailableUnits: ["A3", "A12", "A27", "A44", "A58"], reserveCapacity: 0 },
  { id: "b", name: "Carrinho B", code: "B", prefix: "B", total: 58, available: 54, location: "Armário B · Bloco 1", status: "Pronto", lastCheck: "Hoje, 06:48", accent: "bg-[hsl(var(--accent))]", unavailable: false, unavailableUnits: ["B8", "B19", "B37", "B51"], reserveCapacity: 10 },
  { id: "c", name: "Carrinho C", code: "C", prefix: "C", total: 55, available: 51, location: "Armário C · Bloco 2", status: "Em uso", lastCheck: "Hoje, 06:41", accent: "bg-[hsl(158_43%_43%)]", unavailable: false, unavailableUnits: ["C4", "C16", "C33", "C48"], reserveCapacity: 10 },
];

export const initialTeacherAccounts: TeacherAccount[] = [
  { id: "teacher-marina", ...defaultTeacher, password: undefined, mustSetPassword: true },
  { id: "teacher-rafael", name: "Rafael Nunes", email: "rafael.nunes@campus.edu.br", segment: "Fundamental 2", subject: "Matemática", password: "1234", mustSetPassword: false },
  { id: "teacher-bianca", name: "Bianca Reis", email: "bianca.reis@campus.edu.br", segment: "Fundamental 2", subject: "Geografia", password: "1234", mustSetPassword: false },
];

export const initialWifiPoints: WifiPoint[] = [
  { id: "point-01", name: "Bloco 1 · Térreo", point: "Ponto AP-01", type: "Ponto fixo", location: "Armário de rede · Bloco 1", rack: "Rack B1 · porta 08", status: "Disponível" },
  { id: "point-02", name: "Bloco 1 · 2º andar", point: "Ponto AP-02", type: "Ponto fixo", location: "Armário de rede · Bloco 1", rack: "Rack B1 · porta 14", status: "Disponível" },
  { id: "point-03", name: "Laboratórios · Bloco 2", point: "Ponto AP-03", type: "Ponto fixo", location: "Sala técnica · Bloco 2", rack: "Rack B2 · porta 03", status: "Em manutenção" },
  { id: "point-04", name: "Antena volante 01", point: "Antena AV-01", type: "Antena volante", location: "Armário de conectividade", rack: "Rack móvel · porta 01", status: "Disponível" },
  { id: "point-05", name: "Antena volante 02", point: "Antena AV-02", type: "Antena volante", location: "Armário de conectividade", rack: "Rack móvel · porta 02", status: "Disponível" },
];
...
export function CampusDataProvider({ children }: { children: ReactNode }) {
  const [teacher, setTeacher] = useState<TeacherProfile>(() => ({ ...defaultTeacher, ...readStorage<Partial<TeacherProfile>>(teacherStorageKey, {}) }));
  ...
  const [carts, setCarts] = useState<Cart[]>(() => readStorage<Partial<Cart>[]>(cartStorageKey, initialCarts).map((cart) => ({ ...cart, unavailable: cart.unavailable ?? false, unavailableUnits: cart.unavailableUnits ?? [], reserveCapacity: cart.reserveCapacity ?? (cart.code === "B" || cart.code === "C" ? 10 : 0) }) as Cart));
  const [teacherAccounts, setTeacherAccounts] = useState<TeacherAccount[]>(() => readStorage(teacherAccountsStorageKey, initialTeacherAccounts));
  const [wifiPoints, setWifiPoints] = useState<WifiPoint[]>(() => readStorage(wifiPointsStorageKey, initialWifiPoints));
  const [wifiRooms, setWifiRooms] = useState<typeof initialWifiRooms>(() => readStorage(wifiRoomsStorageKey, initialWifiRooms));
  ...
  const persistTeacherAccounts = (next: TeacherAccount[]) => {
    ...
    window.localStorage.setItem(teacherAccountsStorageKey, JSON.stringify(next));
  };
  ...
  const toggleCartUnitUnavailable = (cartId: string, unit: string) => {
    persistCarts(carts.map((cart) => {
      ...
      return { ...cart, unavailableUnits, available: Math.max(0, cart.total - unavailableUnits.length), status: unavailableUnits.length >= cart.total ? "Indisponível" : cart.status === "Indisponível" ? "Pronto" : cart.status };
    }));
  };
  ...
  const addTeacherAccounts = (entries: Array<Omit<TeacherAccount, "id" | "password" | "mustSetPassword">>) => {
    const next = [...teacherAccounts];
    entries.forEach((entry) => {
      const email = entry.email.trim().toLowerCase();
      if (!email || next.some((account) => account.email === email)) return;
      next.push({ ...entry, email, id: `teacher-${Date.now()}-${next.length}`, mustSetPassword: true });
    });
  ...
  const updateWifiPointStatus = (id: string, status: WifiPoint["status"]) => {
    persistWifiPoints(wifiPoints.map((point) => point.id === id ? { ...point, status } : point));
  };
  const assignMobileAntenna = (pointId: string, room: string | undefined) => {
    persistWifiPoints(wifiPoints.map((point) => {
      if (point.id === pointId) return { ...point, assignedRoom: room, status: room ? "Em uso" : "Disponível" };
      if (room && point.type === "Antena volante" && point.assignedRoom === room) return { ...point, assignedRoom: undefined, status: "Disponível" };
      return point;
    }));
  };
