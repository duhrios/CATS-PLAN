...
export function OperatorLoginPage() {
  ...
  return <div className="flex min-h-[calc(100dvh-76px)] items-center justify-center py-8"><form className="w-full max-w-md space-y-5 rounded-2xl border border-[hsl(var(--card-border))] bg-[hsl(var(--card))] p-6 shadow-lg sm:p-8]" onSubmit={(event) => { event.preventDefault(); const account = authenticateOperator(name, password); if (!account) { setError("Nome ou senha do TI incorretos."); return; } window.localStorage.setItem("controle-carrinhos-role", account.isAdmin ? "admin" : "operator"); window.localStorage.setItem("controle-carrinhos-super-admin", "false"); window.localStorage.setItem("controle-carrinhos-operator-name", account.name); if (account.isAdmin) window.localStorage.setItem("controle-carrinhos-admin-name", account.name); setLocation(account.isAdmin ? "/admin" : "/operador"); }}><div className="flex items-center gap-3"><span className="grid h-11 w-11 items-center justify-center rounded-2xl bg-[hsl(var(--primary))] text-white"><LogIn size={20} /></span><div><h1 className="font-display text-xl font-semibold">Acesso do TI</h1><p className="text-xs text-[hsl(var(--muted-foreground))]">Agenda e movimentação dos carrinhos</p></div></div><Field label="Nome"><input required value={name} onChange={(event) => setName(event.target.value)} className={inputClass} /></Field><Field label="Senha"><input required type="password" value={password} onChange={(event) => setPassword(event.target.value)} className={inputClass} placeholder="Senha do TI" /></Field>{error && <p className="rounded-lg bg-[hsl(var(--destructive)/.1)] p-3 text-xs font-semibold text-[hsl(var(--destructive))]">{error}</p>}<Button className="w-full" type="submit">Entrar</Button><Link href="/" className="block text-center text-xs font-semibold text-[hsl(var(--primary))]">Voltar para acessos</Link></form></div>;
}

export function OperatorPage() {
  const { reservations, movements, carts, movementSettings, updateMovementStatus, markMovementNotAttended, wifiRooms, wifiPoints, assignMobileAntenna, releaseMobileAntenna } = useCampusData();
  const { floorForRoom } = useRoomDirectory();
  const isOperatorAdmin = typeof window !== "undefined" && window.localStorage.getItem("controle-carrinhos-role") === "admin";
  const [warning, setWarning] = useState("");
  const [alarm, setAlarm] = useState<Reservation | null>(null);
  const [view, setView] = useState<"pending" | "completed" | "notAttended" | "future">("pending");
  ...
  useEffect(() => {
    ...
    today.forEach((reservation) => {
      const movement = movements.find((entry) => entry.reservationId === reservation.id);
      const [hour, minute] = reservation.end.split(":").map(Number);
      if (currentMinutes >= hour * 60 + minute && (!movement || movement.status === "Não movido")) {
        markMovementNotAttended(reservation.id);
      }
    });
  }, [now, today, movements, markMovementNotAttended]);
  useEffect(() => {
    ...
    const reservation = today.find((item) => {
      ...
      if (movement?.status === "Concluído" || movement?.status === "Não atendida") return false;
  ...
  useEffect(() => {
    ...
    if (!movement) return;
    const actorName = movementActor();
    const isCurrentActor = movement.movedBy === actorName;
    if (movement.status === "Concluído") {
      alarmStopRef.current?.();
      alarmStopRef.current = null;
      setAlarm(null);
      return;
    }
    if (movement.status === "Movendo" && !isCurrentActor) {
      alarmStopRef.current?.();
      alarmStopRef.current = null;
      setAlarm(null);
      return;
    }
    if (movement.status === "Movendo" && movementSettings.autoComplete && now - new Date(movement.updatedAt).getTime() >= movementSettings.alertIntervalMinutes * 60000) {
      alarmStopRef.current?.();
      alarmStopRef.current = null;
      setAlarm(null);
    }
  }, [alarm, movementSettings, movements, now]);
  ...
  const visibleToday = today.filter((item) => {
    ...
    return movement?.status !== "Concluído" && movement?.status !== "Não atendida";
  });
  const handleAlarmMove = (reservation: Reservation) => {
    ...
    const mobileAntenna = wifiPoints.find((point) => point.type === "Antena volante" && (!point.assignedRoom || point.assignedRoom === reservation.room) && point.status === "Disponível");
    if (roomNeedsAntenna && mobileAntenna) assignMobileAntenna(mobileAntenna.id, reservation.room);
    playMovementAlert();
    updateMovementStatus(reservation.id, "Movendo", false, movementActor(), timing.late);
  };
  const handleAlarmComplete = (reservation: Reservation) => {
    ...
    alarmStopRef.current?.();
    alarmStopRef.current = null;
    updateMovementStatus(reservation.id, "Concluído", false, movementActor(), timing.late);
    setAlarm(null);
  };
  const alarmMovement = alarm ? movements.find((entry) => entry.reservationId === alarm.id) : null;
  const alarmActionLabel = alarmMovement?.status === "Movendo" ? "Concluído" : "Mover carrinho";
  const previousCartReservation = alarm
    ? [...reservations]
        .filter((entry) => entry.cart === alarm.cart && `${entry.date}${entry.start}` < `${alarm.date}${alarm.start}`)
        .sort((a, b) => `${b.date}${b.start}`.localeCompare(`${a.date}${a.start}`))[0]
    : null;
  return <div className="animate-rise space-y-7"><PageHeader eyebrow="Área do TI" title="Agenda de movimentações" description={`Leve os carrinhos até as salas e atualize cada etapa. Disponíveis agora: ${liveAvailability.available} de ${liveAvailability.total} Chromebooks.`} action={isOperatorAdmin ? <Link href="/admin" className="text-xs font-semibold text-[hsl(var(--primary))]">Voltar ao administrador</Link> : undefined} />{alarm && <div role="alert" className="overflow-hidden rounded-[28px] border border-[#d7e4f6] bg-[linear-gradient(135deg,#eaf3ff_0%,#dfeeff_28%,#ffffff_100%)] p-4 text-slate-800 shadow-[0_22px_48px_rgba(15,53,94,0.12)]"><div className="flex items-start justify-between gap-3"><div className="flex items-center gap-3"><div className="grid h-12 w-12 place-items-center rounded-2xl bg-[linear-gradient(135deg,#0f5ec8_0%,#3c8ef8_100%)] ring-1 ring-[#b9d1f0]"><Bell size={20} className="text-white" /></div><div className="min-w-0"><p className="text-[10px] font-bold uppercase tracking-[0.22em] text-[#2f5e8f]">Notificação</p><p className="mt-1 text-[1.8rem] font-black leading-none tracking-[-0.05em] text-[#0f2340]">Mover carrinho</p></div></div><button type="button" onClick={() => setAlarm(null)} className="rounded-full bg-white/80 p-2 text-slate-600 transition hover:bg-white" aria-label="Fechar aviso"><X size={18} /></button></div>    <div className="mt-4 rounded-[22px] border border-[#bfd6f6] bg-[linear-gradient(180deg,#ffffff_0%,#f7fbff_100%)] p-3.5 shadow-[inset_0_1px_0_rgba(255,255,255,0.85)]"><p className="text-[10px] font-bold uppercase tracking-[0.24em] text-[#8a6b1d]">Agendamento</p><div className="mt-2 flex flex-wrap items-center gap-2"><p className="text-xl font-black tracking-[-0.04em] text-[#102b4d]">{alarm.start}–{alarm.end}</p>  <span className="rounded-full border border-[#f0b400] bg-[linear-gradient(135deg,#fff3a6_0%,#ffd84d_100%)] px-2.5 py-1 text-[10px] font-black uppercase tracking-[0.12em] text-[#684900] shadow-[0_2px_8px_rgba(240,180,0,0.2)]">{alarm.cart}</span></div><p className="mt-1 text-base font-semibold text-slate-800">{alarm.className}</p><p className="mt-1 text-sm font-medium text-slate-700">{alarm.room} · {alarm.teacher}</p>{previousCartReservation && <p className="mt-2 rounded-lg border border-[#dfe9f9] bg-[#f3f8ff] px-2.5 py-2 text-xs font-semibold text-[#234b79]">Último uso do carrinho: {previousCartReservation.room} · {floorForRoom(previousCartReservation.room)}</p>}</div><div className="mt-4 flex gap-3"><button type="button" onClick={() => alarmMovement?.status === "Movendo" ? handleAlarmComplete(alarm) : handleAlarmMove(alarm)} className="flex-1 rounded-xl bg-[linear-gradient(135deg,#2d8ef5_0%,#0f5ec8_100%)] px-4 py-3 text-sm font-bold text-white shadow-[0_12px_28px_rgba(15,94,200,0.28)] transition hover:brightness-110">{alarmActionLabel}</button></div><p className="mt-3 text-[11px] text-slate-600">O aviso segue as configurações definidas pela administração.</p></div>}{warning && <div role="alert" className="rounded-xl border border-[hsl(var(--destructive)/.4)] bg-[hsl(var(--destructive)/.08)] p-4 text-sm font-semibold text-[hsl(var(--destructive))]">{warning}</div>}{view === "future" ? <SectionCard title="Agendamentos futuros" eyebrow="Próximas movimentações"><FutureReservations reservations={reservations} movements={movements} onViewChange={(nextView) => setView(nextView)} /></SectionCard> :   <SectionCard title="Movimentações de hoje" eyebrow={`${visibleToday.length} agendamento${visibleToday.length === 1 ? "" : "s"} ${view === "completed" ? "concluído" : view === "notAttended" ? "não atendido" : "para atender"}`}><div className="flex flex-wrap gap-2 border-b border-[hsl(var(--border))] px-5 pt-4 sm:px-6"><button type="button" onClick={() => setView("pending")} className={`border-b-2 px-1 pb-3 text-xs font-bold ${view === "pending" ? "border-[hsl(var(--primary))] text-[hsl(var(--primary))]" : "border-transparent text-[hsl(var(--muted-foreground))]"}`}>Para atender</button>  <button type="button" onClick={() => setView("completed")} className={`border-b-2 px-1 pb-3 text-xs font-bold ${view === "completed" ? "border-[hsl(var(--primary))] text-[hsl(var(--primary))]" : "border-transparent text-[hsl(var(--muted-foreground))]"}`}>Concluído</button><button type="button" onClick={() => setView("notAttended")} className={`border-b-2 px-1 pb-3 text-xs font-bold ${view === "notAttended" ? "border-[hsl(var(--primary))] text-[hsl(var(--primary))]" : "border-transparent text-[hsl(var(--muted-foreground))]"}`}>Não atendidas</button><button type="button" onClick={() => setView("future")} className="border-b-2 border-transparent px-1 pb-3 text-xs font-bold text-[hsl(var(--muted-foreground))]">Futuras</button></div>{visibleToday.length === 0 ? <p className="p-6 text-sm text-[hsl(var(--muted-foreground))]">  {view === "completed" ? "Nenhuma movimentação concluída hoje." : view === "notAttended" ? "Nenhuma movimentação não atendida hoje." : "Nenhuma movimentação para hoje."}</p> : <div className="space-y-4 p-4 sm:p-5">{visibleToday.map((item) => { const movement = movements.find((entry) => entry.reservationId === item.id); const status = movement?.status ?? "Não movido"; const roomWifi = wifiRooms.find((room) => room.room === item.room); const roomNeedsAntenna = !roomWifi || !roomWifi.hasWifi;   const mobileAntenna = wifiPoints.find((point) => point.type === "Antena volante" && (!point.assignedRoom || point.assignedRoom === item.room) && point.status === "Disponível"); const previousCartReservation = reservations.filter((entry) => entry.cart === item.cart && `${entry.date}${entry.start}` < `${item.date}${item.start}`).sort((a, b) => `${b.date}${b.start}`.localeCompare(`${a.date}${a.start}`))[0]; return <div key={item.id} className="rounded-[24px] border border-[#e7edf5] bg-[linear-gradient(180deg,#ffffff_0%,#f5f7fb_100%)] p-4 shadow-[0_10px_24px_rgba(15,23,42,0.04)]"><div className="flex items-start justify-between gap-3"><div className="flex min-w-0 items-start gap-3"><div className="grid h-10 w-10 shrink-0 place-items-center rounded-full bg-[#edf3fc] text-[hsl(var(--primary))] ring-1 ring-[#dfe8f3]"><MoveRight size={18} /></div><div className="min-w-0 flex-1"><p className="text-[10px] font-bold uppercase tracking-[0.2em] text-slate-500">Agendamento</p><div className="mt-2 flex flex-wrap items-center gap-2"><p className="text-[1.55rem] font-black leading-none tracking-[-0.06em] text-slate-900">{item.start}–{item.end}</p>  <span className="rounded-full border border-[#f0b400] bg-[#ffd84d] px-2.5 py-1 text-[10px] font-black uppercase tracking-[0.12em] text-[#684900] shadow-[0_2px_8px_rgba(240,180,0,0.2)]">{item.cart}</span></div><p className="mt-1 text-base font-semibold text-slate-800">{item.className}</p>  <p className="mt-1 text-sm text-slate-700">{item.room} · {floorForRoom(item.room)} · Professor: {item.teacher}</p>{movement?.notReceived && <p className="mt-2 rounded-lg bg-[hsl(var(--destructive)/.08)] p-2 text-xs font-semibold text-[hsl(var(--destructive))]">Carrinho não movimentado — o professor não recebeu. Solicitação {movement.requestCount > 1 ? `repetida (${movement.requestCount}x)` : "registrada"}.</p>}{roomNeedsAntenna &&             <p className="mt-2 rounded-lg border border-[#d7dee8] bg-[#f3f5f8] p-2 text-xs font-semibold text-black">Sala sem conexão fixa · {mobileAntenna ? `${mobileAntenna.name} (${mobileAntenna.point})` : "nenhuma antena volante disponível"}</p>}</div></div><StatusPill status={status} /></div>{view === "pending" && <div className="mt-4 flex flex-wrap gap-2"><Button size="sm" variant={status === "Movendo" ? "primary" : "secondary"} onClick={() => { const timing = canUpdateMovement(item.start, item.end); if (!timing.allowed) return; if (roomNeedsAntenna && mobileAntenna) assignMobileAntenna(mobileAntenna.id, item.room); playMovementAlert(); updateMovementStatus(item.id, "Movendo", false, movementActor(), timing.late); }}><Clock3 size={14} /> Movendo</Button><Button size="sm" variant={status === "Concluído" ? "primary" : "secondary"} onClick={() => { const timing = canUpdateMovement(item.start, item.end); if (!timing.allowed) return; if (roomNeedsAntenna && mobileAntenna) assignMobileAntenna(mobileAntenna.id, item.room); updateMovementStatus(item.id, "Concluído", false, movementActor(), timing.late); }}><Check size={14} /> Concluído</Button></div>}</div>; })}</div>}</SectionCard>}</div>;
}
