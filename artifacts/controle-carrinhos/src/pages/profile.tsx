import { Check, FileSpreadsheet, FileText, KeyRound, Loader2, Mail, Plus, RotateCcw, Upload, UserRound, ShieldCheck, Pencil, X } from "lucide-react";
...
export function TeacherProfilePage({ admin = false }: { admin?: boolean }) {
  ...
  const deleteSelectedTeachers = () => {
    selectedTeacherIds.forEach((id) => deleteTeacherAccount(id));
    setSelectedTeacherIds([]);
  };

  const deleteSelectedOperators = () => {
    selectedOperatorIds.forEach((id) => deleteOperatorAccount(id));
  ...
  const openAdminEditor = (account: (typeof adminAccounts)[number]) => {
    setEditingAdminId(account.id);
    setEditingAdminForm({ name: account.name, password: "" });
  };
  const saveAdminEdit = (event: FormEvent<HTMLFormElement>) => {
    event.preventDefault();
    if (!editingAdminId || !editingAdminForm.name.trim() || editingAdminForm.password.length < 6) return;
    updateAdminAccount(editingAdminId, editingAdminForm.name.trim(), editingAdminForm.password);
  ...
  const saveOperatorEdit = (event: FormEvent<HTMLFormElement>) => {
    event.preventDefault();
    ...
    updateOperatorAccount(account.id, editingOperatorForm.name.trim(), editingOperatorForm.password, Boolean(account.isAdmin));
  ...
  return (
    <div className="animate-rise space-y-7">
        ...
        <div className="space-y-3 p-5 sm:p-6">
          {selectedTeacherIds.length > 0 && (
            <div className="flex justify-end">
              <Button size="sm" variant="ghost" onClick={deleteSelectedTeachers} className="text-[hsl(var(--destructive))] hover:text-[hsl(var(--destructive))]">
      ...
      <SectionCard title="Usuários TI" eyebrow="Criar acessos para movimentação" action={<Button size="sm" onClick={() => { setOperatorError(""); setOperatorModalOpen(true); }}><Plus size={15} /> Criar usuário TI</Button>}>
      ...
      {adminModalOpen && <Modal title="Atualizar acesso do administrador" onClose={() => setAdminModalOpen(false)}><form className="space-y-4 p-5 sm:p-6" onSubmit={saveAdmin}><Field label="Nome do administrador"><input value={adminForm.name} onChange={(event) => setAdminForm({ ...adminForm, name: event.target.value })} className={inputClass} placeholder="Nome do administrador" /></Field><Field label="Nova senha" hint="Use pelo menos 6 caracteres."><input required minLength={6} type="password" value={adminForm.password} onChange={(event) => setAdminForm({ ...adminForm, password: event.target.value })} className={inputClass} placeholder="Nova senha" /></Field><div className="flex justify-end gap-2"><Button type="button" variant="ghost" onClick={() => setAdminModalOpen(false)}>Cancelar</Button><Button type="submit"><Check size={15} /> Atualizar</Button></div></form></Modal>}
      {editingAdminId && <Modal title="Editar administrador" onClose={() => setEditingAdminId(null)}><form className="space-y-4 p-5 sm:p-6" onSubmit={saveAdminEdit}><Field label="Nome do administrador"><input required minLength={3} value={editingAdminForm.name} onChange={(event) => setEditingAdminForm({ ...editingAdminForm, name: event.target.value })} className={inputClass} /></Field><Field label="Nova senha" hint="Use pelo menos 6 caracteres."><input required minLength={6} type="password" value={editingAdminForm.password} onChange={(event) => setEditingAdminForm({ ...editingAdminForm, password: event.target.value })} className={inputClass} placeholder="Nova senha" /></Field><div className="flex justify-end gap-2"><Button type="button" variant="ghost" onClick={() => setEditingAdminId(null)}>Cancelar</Button><Button type="submit"><Check size={15} /> Salvar alterações</Button></div></form></Modal>}
...
export function TeacherLoginPage() {
  ...
  const signIn = (event: FormEvent<HTMLFormElement>) => {
    ...
    window.localStorage.setItem("controle-carrinhos-role", "user");
